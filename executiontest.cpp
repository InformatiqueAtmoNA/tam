...
void ExecutionTest::getInfosEquipements()
{
    ...
    while (iterator.hasNext()) {
        ...
        QVector<QPointer<MesureIndividuelle> > tabMesuresParCycle;
        m_tabMesuresParCycle.insert(idAnalyseur,tabMesuresParCycle);

        QVector<QPointer<MesureIndividuelle> > tableauMoyennesParPhase;
...
void ExecutionTest::enregistrerMesures()
{
    ...
    QMapIterator<ushort,QPointer<MesureIndividuelle> > it_tableauMesures(m_tabMesuresIndividuelles);

    while (it_tableauMesures.hasNext()) {
        it_tableauMesures.next();

        QPointer<MesureIndividuelle> mesures = it_tableauMesures.value();
...
void ExecutionTest::enregistrerMoyenneMesures()
{
    ...
    QMapIterator<ushort,QVector<QPointer<MesureIndividuelle> > > it_tabMesuresParCycle(m_tabMesuresParCycle);
    while(it_tabMesuresParCycle.hasNext()) {
        it_tabMesuresParCycle.next();

        QVector<QPointer<MesureIndividuelle> > tableaumesureParCycle = it_tabMesuresParCycle.value();

        QPointer<MesureIndividuelle> moyenneMesure(new MesureIndividuelle());
        for(ushort j=0;j<3;j++) {
            ushort nbMesuresMoyennees=0;
            float sommeMesuresIndividuelles=0;
            for(ushort i=0;i<tableaumesureParCycle.count();i++) {
                QPointer<MesureIndividuelle> mesure(tableaumesureParCycle.at(i));
...
void ExecutionTest::verifierCritereArret()
{
    ...
    while(it_tabMoyennesMesuresParPhase.hasNext()) {
        ...
        QVector<QPointer<MesureIndividuelle> > tableauMoyennesMesuresParPhase = it_tabMoyennesMesuresParPhase.value();
...
void ExecutionTest::testerPhasesRestantes()
{
    ...
    while(it_tabMoyennesMesuresParPhase.hasNext()) {
        it_tabMoyennesMesuresParPhase.next();
        QVector<QPointer<MesureIndividuelle> > vecteurVide;
...
void ExecutionTest::testerCyclesMesures()
{
    ...
    while(it_tabMesuresParCycle.hasNext()) {
        ...
        QVector<QPointer<MesureIndividuelle> > tabMesuresParCycle = it_tabMesuresParCycle.value();
        for(int i=0;i<tabMesuresParCycle.count();i++) {
            QPointer<MesureIndividuelle> mesureIndividuelle = tabMesuresParCycle.at(i);
